...
public:
    ...
    virtual void destroyObject(const ObjectHandle& handle)
    {
        if (NETWORK->isConnected())
            NETWORK->BroadcastDestruction(this->get(handle), RakNet::UNASSIGNED_SYSTEM_ADDRESS);
        ObjectFactory<T>::destroyObject(handle);
    }
    virtual void destroyAt(ObjectHandle::IndexType index)
    {
        if (NETWORK->isConnected())
            NETWORK->BroadcastDestruction(this->getAt(index), RakNet::UNASSIGNED_SYSTEM_ADDRESS);
