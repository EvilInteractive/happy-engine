...
bool HappyCooker::cook(const std::string& input, const std::string& output)
{
    ...
    if (m_ModelCookData.size() > 0)
    {
        ...
        if (stream.open(m_ModelExportPath + output + ".binobj", io::BinaryStream::Write) == false)
        {
            HE_ERROR("Opening bin stream failed: " + m_ModelExportPath + output + ".binobj");
        }
        else
            cookBinObj(stream);
    }
    ...
    {
        io::BinaryStream convexStream;
        if (convexStream.open(m_PhysicsExportPath + output + ".pxcv", io::BinaryStream::Write) == false)
        {
            HE_ERROR("Opening bin stream failed: " + m_PhysicsExportPath + output + ".pxcv");
        }
        else
    ...
    {
        io::BinaryStream concaveStream;
        if (concaveStream.open(m_PhysicsExportPath + output + ".pxcc", io::BinaryStream::Write) == false)
        {
            HE_ERROR("Opening bin stream failed: " + m_PhysicsExportPath + output + ".pxcc");
        }
        else
